...
if(!empty($_POST)){
  if($_POST['cpanelId']=="")
  {
    $checkArticleInfos =$db->display('cpanels',' url="'.$_POST['url'].'" and login='.$_POST['login']);
    ...
    $data = array(
        ...
        $backlinks = $db->insert('cpanels',$data);
      }
        header("location:manage-cpanels.php");
  }
  else
  {
    $cpanelId=$_POST['cpanelId'];
    // $checkArticleInfos =$db->display('cpanels',' url="'.$_POST['url'].'" and login="'.$_POST['login'].'"');
    ...
    // if($checkArticleInfos->num_rows > 0){
      $data = array(
        ...
        $db->update('cpanels',$data,'id='.$cpanelId);
        header("location:manage-cpanels.php");
    //   }else{
    //     $errorMsg = 'can update record';
    //     header("location:manage-cpanels.php?errorMsg=".$errorMsg."");
    //   }
  }
}
if($_GET['id']!=''){
  $ArticleInfosQuery=$db->display('cpanels','id='.$_GET['id']);
...
<head>
    ...
    <title>Conversions - Client Panel | Cpanel Add</title>
                    ...
                    <div class="col-md-10">
                        <div class="card card-primary">
                            <div class="card-header">
                                <h3 class="card-title">Cpanel Details</h3>
                  ...
                  <i class="fas fa-minus"></i>
                            ...
                            <div class="card-body">
                                <form action="" method="post">
                                    ...
                                    <div class="row">
                                        <div class="col-3">
                                            <a href="manage-cpanels.php" class="btn btn-secondary">Cancel</a>
                                            <?php if($_GET['id']!=''){?>
                                            <input type="submit" value="Update Cpanel"
                                                class="btn btn-success float-right">
                                            <?php }else{?>
                                            <input type="submit" value="Create Cpanel"
                                                class="btn btn-success float-right">
                                            <?php }?>
                                        </div>
                                    </div>
